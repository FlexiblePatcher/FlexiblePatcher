...
void pWriteMyData(void *Data, DWORD Size)
{
	BYTE *Pointer;
...
void pReadMyData(void *Data, DWORD Size)
{
	if (Heap.Size <= Size) {
...
void ApplyPatches(void)
{
	...
	const array_segment *Segment;

	Blob = CreateFile("patches.fpd", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (version != VERSION) {
		if (version < VERSION) {
			MessageBox(NULL, "The game can't start because patches.fpd is older than Plugin_FlexiblePatcher.dll. Try repatching the data file to fix this problem.", "TRNG Engine", MB_OK);
		} else {
			MessageBox(NULL, "The game can't start because patches.fpd is newer than Plugin_FlexiblePatcher.dll. Try reinstalling the plugin to fix this problem.", "TRNG Engine", MB_OK);
		}
		TerminateProcess(GetCurrentProcess(), 0);
	}
	...
	for (Number = 0; Number < patch_selected_size; ++Number) {
		if (patch_selected[Number]) {
			Count = data_address_segment[Number].count;
			Address = &data_address[data_address_segment[Number].start];
			Segment = &data_buffer_segment[data_address_segment[Number].start];
			for (Index = 0; Index < Count; ++Index)
				memcpy((void *) Address[Index], &data_buffer[Segment[Index].start], Segment[Index].count);
			Count = parameter_address_segment[Number].count;
			Address = &parameter_address[parameter_address_segment[Number].start];
			Segment = &parameter_buffer_segment[parameter_address_segment[Number].start];
			for (Index = 0; Index < Count; ++Index)
				memcpy((void *) Address[Index], &parameter_buffer[Segment[Index].start], Segment[Index].count);
		}
	}
	*Target = 0x49F18B;
}
                       ...
                       LPVOID lpReserved)
{
    ...
    switch (ul_reason_for_call)
	{
		case DLL_PROCESS_ATTACH:
			...
			if (!GetModuleHandle("Tomb_NextGeneration.dll")) {
				ApplyPatches();
				return FALSE;
			}
